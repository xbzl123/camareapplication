package com.example.camareapplication.h264encode;
...
public class H264Encoder {
    ...
    private static int yuvqueuesize = 1200;
    private final ArrayBlockingQueue<byte[]> YUVQueue = new ArrayBlockingQueue<>(yuvqueuesize);
    ...
    public void setHandler(Handler mHandler, ThreadPoolExecutor mExecutor){
        handler = mHandler;
        executor = mExecutor;
    }
    ...
    public void StartEncoderThread(){
        ...
        Runnable task = () -> {
            ...
            long taskTime = System.currentTimeMillis();
            {
                ...
                while (isRuning) {
                    if (YUVQueue.size() > 0){
                        Log.e("StartEncoderThread","size = "+YUVQueue.size());
                        //从缓冲队列中取出一帧
